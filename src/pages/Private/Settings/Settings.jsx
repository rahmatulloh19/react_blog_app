  ...
  const handleSubmit = (evt) => {
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          setModal(false);
          dispatch(meSlice.actions.setMe(res.data));
  ...
  return (
    <div className="p-5 d-flex align-items-center justify-content-center h-100">
      <div className="bg-light position-relative d-flex flex-column align-items-center border rounded p-4 gap-2">
        {me.img ? (
          <img className="rounded-circle flex-grow-0 position-absolute user__img" width={70} height={70} src={me.img} alt="" />
        ) : (
          <FaRegCircleUser className="rounded-circle flex-grow-0 position-absolute user__img bg-white" style={{ width: "70px", height: "70px" }} />
        )}
        ...
        <Modal closeModal={setModal} title={t("settings.modalTitle")}>
          <form className="d-flex flex-column" onSubmit={handleSubmit}>
            <label className="mb-3">
              ...
              <input className="form-control mt-2" type="text" placeholder={t("settings.firstNameHolder")} ref={firstName} defaultValue={me.first_name ? me.first_name : ""} />
            </label>
            <label className="mb-3">
              {t("settings.modalLastName")}
              <input className="form-control mt-2" type="text" placeholder={t("settings.lastNameHolder")} defaultValue={me.last_name ? me.last_name : ""} ref={lastName} />
            </label>
            <label className="mb-3">
              {t("settings.modalEmail")}
              <input className="form-control mt-2" type="text" placeholder={t("settings.emailHolder")} defaultValue={me.email ? me.email : ""} ref={email} required />
            </label>
            <label className="mb-3">
              {t("settings.modalPassword")}
              <input className="form-control mt-2" type="text" placeholder={t("settings.passwordHolder")} ref={password} required />
            </label>
            <label className="mb-3">
              {t("settings.modalImage")}
              <input className="form-control mt-2" type="url" placeholder={t("settings.imgHolder")} ref={image} />
            </label>
            <div className="d-flex gap-2">
              <button className="btn btn-outline-danger flex-grow-1" type="button" onClick={handleDelete}>
