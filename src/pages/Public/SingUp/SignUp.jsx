...
export const SignUp = () => {
  ...
  useEffect(() => {
    if (!emailError.length && !passwordError.length && !lastNameError.length && !firstNameError.length) {
      setStatusForm(true);
    } else {
      setStatusForm(false);
    }
  ...
  return (
    <>
      <div className="d-flex align-items-center position-relative">
        ...
        <div className=" flex-grow-1 d-flex flex-column align-items-center h-100 " style={{ paddingTop: "60px" }}>
          ...
          <div className="inner shadow p-3 py-4 w-50">
            ...
            <form className="w-100 d-flex flex-column gap-3" onSubmit={handleSubmit}>
              <label>
                <input
                  className={`form-control px-3 py-2 ${firstNameTouched && firstNameError && "is-invalid"} ${!firstNameError && "is-valid"}`}
                  type="text"
                ...
                />
                <span className={`${firstNameError ? "invalid-feedback" : "valid-feedback"}`}>{firstNameError ? firstNameError : t("signUp.valid")}</span>
              </label>
              <label>
                <input
                  className={`form-control px-3 py-2 ${lastNameTouched && lastNameError && "is-invalid"} ${!lastNameError && "is-valid"}`}
                  type="text"
                ...
                />
                <span className={`${lastNameError ? "invalid-feedback" : "valid-feedback"}`}>{lastNameError ? lastNameError : t("signUp.valid")}</span>
              </label>
              <label>
                <input
                ...
                />
                <span className={`${emailError ? "invalid-feedback" : "valid-feedback"}`}>{emailError ? emailError : t("signUp.valid")}</span>
              </label>
              <label>
                <input
                ...
                />
                <span className={`${passwordError ? "invalid-feedback" : "valid-feedback"}`}>{passwordError ? passwordError : t("signUp.valid")}</span>
              </label>
              <label>
                <input className="form-control px-3 py-2" type="url" placeholder={t("signUp.imgHolder")} ref={userImg} onBlur={() => setImgTouched(true)} />
                <span>{imgTouched && t("signUp.imgFeedBack")}</span>
              </label>

              <button className={`btn btn-success mb-1 ms-auto ${!statusForm ? "disabled" : ""}`} type="submit">
                {t("signUp.btn")}
              </button>
